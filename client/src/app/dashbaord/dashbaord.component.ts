...
export class DashbaordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // ✅ read query param to open consumer panels like cart/wishlist/orders
    this.qpSub = this.route.queryParamMap.subscribe(qp => {
      const panel = (qp.get('panel') || '').toLowerCase();

      if (this.role === 'CONSUMER') {
        if (panel === 'cart') this.consumerPanel = 'cart';
        else if (panel === 'wishlist') this.consumerPanel = 'wishlist';
        else if (panel === 'orders') this.consumerPanel = 'orders';
        this.refreshWishlistCart();
      }
    });
  ...
  showConsumerPanel(panel: 'orders' | 'wishlist' | 'cart'): void {
    ...
    // ✅ keep URL in sync (so navbar "Cart" works cleanly)
